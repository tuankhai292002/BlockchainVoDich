    ...
    socket.on("message", message => {
        ...
        switch(_message.type) {
            case "TYPE_REPLACE_CHAIN":
                ...
                if (newBlock.prevHash !== JeChain.getLastBlock().prevHash) { //giả sử mình vùa đào được block thì người khác gửi yêu cầu add block mới, nếu 2 mã prevhash của block đấy mà khacs
                    for (let i = 0; i < n; i++) {
                ...
                } else if (!checked.includes(JSON.stringify([newBlock.prevHash, JeChain.chain[JeChain.chain.length-2].timestamp || ""]))) { //nếu 2 mã prevhash của block đấy mà gi
